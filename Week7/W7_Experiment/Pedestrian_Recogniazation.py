...
def tune_SVM_parameters(x, y, test_data):
    """
    自动调参 SVM，并绘制最佳模型的 ROC 曲线。

    参数
    ---
    x, y: 训练数据及标签。
    test_data: 测试数据 (hog_features, labels)。
    """
    print("Start GridSearchCV for SVM...")
    ...
    grid_search = GridSearchCV(SVC(probability=True), param_grid, cv=3, scoring='roc_auc', verbose=1)
    grid_search.fit(x, y)
    ...
    # 测试并绘制 ROC 曲线
    hog_features, labels = test_data
    prob = best_model.predict_proba(hog_features)[:, 1]
...
def read_images(pos_dir, neg_dir,
    ...
    for neg_file in tqdm(neg_img_files,
        ...
        # 均匀裁剪: 在left和top方向上均匀采样neg_area_count个位置
        if neg_area_count > 1:
            left_list = np.linspace(0, left_max, num=neg_area_count, dtype=int)
            top_list = np.linspace(0, top_max, num=neg_area_count, dtype=int)
        else:
            left_list = np.array([left_max // 2])
            top_list = np.array([top_max // 2])
        for left, top in zip(left_list, top_list):
...
def read_training_data():
    ...
    读取训练数据。
    """
    pos_dir = '/Users/feng/Desktop/计算机视觉/Week7/W7_Experiment/INRIADATA/normalized_images/train/pos'
    neg_dir = '/Users/feng/Desktop/计算机视觉/Week7/W7_Experiment/INRIADATA/normalized_images/train/neg'
    neg_area_count = 10
    description = '训练'
...
def read_test_data():
    ...
    读取测试数据。
    """
    pos_dir = '/Users/feng/Desktop/计算机视觉/Week7/W7_Experiment/INRIADATA/original_images/test/pos'
    neg_dir = '/Users/feng/Desktop/计算机视觉/Week7/W7_Experiment/INRIADATA/original_images/test/neg'
    neg_area_count = 10
    description = '测试'
    return read_images(pos_dir, neg_dir,
                       neg_area_count, description)
...
def test_SVM(SVM, test_data, show_stats=False):
    ...
    if show_stats:
        ...
        plt.show()
        # 标准 ROC 曲线
        fpr2, tpr2, _ = metrics.roc_curve(labels, prob)
        plt.figure()
...
def detect_multiple_images(SVM, dir):
    ...
    for file in files:
        ...
        boxes = detect_pedestrian(SVM, file_path, show_img=False)
        shown_img = np.array(img)
        for box in boxes:
            shown_img = rectangle(shown_img,
                                  pt1=(box[0], box[1]),
        ...
        cv2.imwrite(str(out_path), (shown_img * 255).astype('uint8'))
...
if __name__ == '__main__':
    ...
    # GridSearchCV 自动调参并画 ROC 曲线
    tune_SVM_parameters(x, y, test_data)

    detect_multiple_images(SVM,  # 用SVM模型识别图片
                           '/Users/feng/Desktop/计算机视觉/Week7/W7_Experiment/INRIADATA/original_images/test/pos')